...
export const getCourseProgress = cache(async () => {
    ...
    return {
        ...
        activceLessonId: firstUncompletedLesson?.id,
    }
});
...
export const getLesson = cache (async (id?: number) => {
    ...
    const lessonId = id || courseProgress?.activceLessonId;
...
export const getLessonPercentage = cache(async () => {
    ...
    if(!courseProgress?.activceLessonId){
        return 0;
    }
    const lesson = await getLesson(courseProgress.activceLessonId);
